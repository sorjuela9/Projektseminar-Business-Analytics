...
#---------------------Sebastian------------------------------
        #9. Stronger Constraint for each direction

        for direction in directions:
            num_trips = len(direction.getTrips())
            for vehicle in fleet:
                vehicle_id = vehicle.getID()

                self.__model += pulp.lpSum(self.__z[trip.getID(), vehicle_id] for trip in direction.getTrips()) <= \
                                num_trips * self.__y[vehicle_id], \
                    f"VehicleUsage_{vehicle_id}_Direction_{direction.getLine()}_{direction.getType()}"
#--------------------Sebastian---------------------------------
