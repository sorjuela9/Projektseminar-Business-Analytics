...
###Für den Gurobi Solver, Mit sstvsA ausführen!!!
...
#))
        ...
        if pulp.LpStatus[self.__model.status] == 'Optimal':
            ...
            for timetable_key, data in timetable_by_line_direction.items():
                ...
                for trip in trips_sorted:
                    ...
                    print(f"  Trip ID: {trip.getID()}, Time Window: {time_window}, "
                        ...
                        f"Vehicle ID: {vehicle_id_display}")
            ...
            def visualize_timetable(timetable_by_line_direction):
                ...
                # Initialisiere Graph und Positionen
                G = nx.DiGraph()
                pos = {1: (0, 0), 2: (1, 0)}  # Feste Positionen für Knoten

                # Für jedes Fahrzeug Trips und Edges verarbeiten
                all_edges = []
                edge_labels = {}
                for timetable_key, data in timetable_by_line_direction.items():
                    for trip in data["trips"]:
                        # Bestimme Start und Ende der Knoten
                        start_node = 1 if "inline" in timetable_key.lower() else 2
                        end_node = 2 if start_node == 1 else 1
                        ...
                        # Erzeuge Edge-Daten
                        all_edges.append((start_node, end_node, trip_id, start_time, end_time))

                        # Kanten-Label vorbereiten
                        if (start_node, end_node) not in edge_labels:
                            edge_labels[(start_node, end_node)] = []
                        edge_labels[(start_node, end_node)].append(
                            f"{trip_id}: {seconds_to_time(start_time)} - {seconds_to_time(end_time)}"
                        )

                        # Fahrzeuge für Farbcodierung
                        for vehicle_id in data["vehicles"]:
                            if vehicle_id not in vehicle_colors:
                                vehicle_colors[vehicle_id] = available_colors[color_index % len(available_colors)]
                                color_index += 1
                ...
                # Zeichnen der Knoten
                nx.draw_networkx_nodes(G, pos, node_color='lightgray', node_size=700)
                nx.draw_networkx_labels(G, pos, labels={node: f"Station {node}" for node in pos}, font_size=10, font_color="black")

                # Zeichnen der Kanten mit gebogenen Pfeilen
                ax = plt.gca()
                for i, (start_node, end_node, trip_id, start_time, end_time) in enumerate(all_edges):
                    # Berechne Krümmung basierend auf Index
                    curvature = 0.2 + (i % 5) * 0.05  # Krümmung leicht variieren
                    arrow_color = vehicle_colors[list(data["vehicles"])[0]]   # Farbe basierend auf erstem Fahrzeug

                    # Zeichne den gebogenen Pfeil
                    arrow = FancyArrowPatch(
                        posA=pos[start_node], posB=pos[end_node],
                        connectionstyle=f"arc3,rad={curvature}",  # Krümmung
                        color=arrow_color, linewidth=2, arrowstyle='-|>', mutation_scale=15
                    )
                    ax.add_patch(arrow)

                # Kanten-Beschriftung hinzufügen (zusammengefasst pro Edge)
                for edge, labels in edge_labels.items():
                    label_text = "\n".join(labels)  # Alle Labels zusammenführen
                    mid_x = (pos[edge[0]][0] + pos[edge[1]][0]) / 2
                    mid_y = (pos[edge[0]][1] + pos[edge[1]][1]) / 2 + 0.1  # Offset für bessere Sichtbarkeit
                    plt.text(mid_x, mid_y, label_text, fontsize=8, color="darkblue", ha="center")

                # Legende für Fahrzeuge
                legend_handles = [
                    Line2D([0], [0], color=color, lw=2, label=f"Vehicle {vehicle_id}")
                    for vehicle_id, color in vehicle_colors.items()
                ]
                plt.legend(handles=legend_handles, loc='upper left')

                # Titel und Anzeige
                plt.title("Trip Visualization with All Vehicles and Trips")
                plt.axis('off')
                plt.show()

            # Call the visualization function with your dataset
            visualize_timetable(timetable_by_line_direction)
        '''
